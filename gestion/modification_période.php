...
if(isset($_GET["annee_per"])){ 
    ...
    if(isset($_POST["submit"])){ // Debut de la inscription
        ...
        $resultat_annee = $req_recup_annee_existe->fetch();
    if($resultat_annee == 0){
    if(!empty($annee_per) && !empty($forfait_km_per)){
        
            $req_update = $dbh->prepare("UPDATE periode SET annee_per = ? , forfait_km_per = ? WHERE annee_per = ? ");
            $req_update->execute(array($annee_per,$forfait_km_per,$annee)); 
